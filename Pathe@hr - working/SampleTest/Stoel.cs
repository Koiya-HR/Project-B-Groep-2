...
namespace SampleTest
{
...

    {
        ...
        public static string makeChairName(int givenRow, int givenCol)
        {
            string name = ((givenRow + 1).ToString().Length, (givenCol + 1).ToString().Length) switch
            {
                (2, 2) => $"{givenRow + 1}-{givenCol + 1}",
                (1, 1) => $"0{givenRow + 1}-0{givenCol + 1}",
                (1, 2) => $"0{givenRow + 1}-{givenCol + 1}",
                (2, 1) => $"{givenRow + 1}-0{givenCol + 1}",
                _ => $"{givenRow + 1}-{givenCol + 1}"
            };
            return name;
        }
