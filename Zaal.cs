...
public class Zaal
{
    ...
    public void checkTicketExists()
    {
        ...
        // Try parsing JSON and handle exceptions
        try
        {
            // Try parsing the JSON string to a JArray
            JArray jsonArray = JArray.Parse(jsonData);

            // Check if the parsed token is an empty array
            if (jsonArray.Count == 0)
            {
                remainingTime = maxRemainingTime;
                return;
            }
            // If it's not empty, you can handle the case where it's not empty
            // For example, return or do further processing
            return;
        }
        catch (JsonReaderException)
        {
            // Handle invalid JSON format
            // For example, if the file is not valid JSON format
            Console.WriteLine("Invalid JSON format in the file.");
            return;
        }
    }
